...
async def test_create_log(client: httpx.Client, test_log_schema: LogCreateSchema):
    """
    Test to verify that the POST endpoint for creating a log successfully creates a log in the database.
    """

    response = await client.post("/logs/", json=test_log_schema.model_dump())
    assert response.status_code == status.HTTP_201_CREATED


async def test_read_valid_log_uid(client: httpx.Client, test_log: Log):
    """
    ...
    response = await client.get(f"/logs/{log.uid}")
    assert response.status_code == status.HTTP_200_OK
    assert response.json().get("data").get("uid") == log.uid


async def test_read_invalid_log_uid(client: httpx.Client):
    """
    ...
    """
    response = await client.get("/logs/invalid_log_uid")
    assert response.status_code == NotFoundError().error.status_code
    assert response.json().get("detail") == NotFoundError().error.detail


async def test_read_log_list(client: httpx.Client, test_create_log_list: list[Log]):
    """
...
async def test_read_log_list_with_tag(
    client: httpx.Client, test_create_log_list: list[Log]
):
    """
    ...
    """
    response = await client.get("/logs/tag/test_tag")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json().get("data")) == len(test_create_log_list)


async def test_read_log_list_with_empty_tag(client: httpx.Client):
    """
    ...
    response = await client.get("/logs/tag/empty_tag")
    assert response.status_code == status.HTTP_200_OK
...
async def test_read_log_list_with_level(
    ...
    response = await client.get(f"/logs/level/{Level.INFO}")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json().get("data")) == len(test_create_log_list)


async def test_read_log_list_with_empty_level(client: httpx.Client):
    """
    Test to verify that the GET endpoint for retrieving logs by level
    returns an empty list when the specified level does not exist.
    """
    response = await client.get(f"/logs/level/{Level.DEBUG}")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json().get("data")) == 0


async def test_read_log_list_with_invalid_level(client: httpx.Client):
    """
    Test to verify that the GET endpoint for retrieving logs by level
...
async def test_read_log_list_with_group_path(
    client: httpx.Client, grouped_logs: list[Log]
):
    """
    ...
    """
    response = await client.get("/logs/group/root-section/")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json().get("data")) == 1


async def test_read_log_list_with_empty_group_path(client: httpx.Client):
    """
    Test to verify that the GET endpoint for retrieving logs by group path
    returns an empty list when the specified group path does not exist.
    """
    response = await client.get("/logs/group/empty-group-path/")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json().get("data")) == 0
...
async def test_read_log_list_with_group_path_children(
    ...
    """
    response = await client.get("/logs/group/root-section/children/")
    assert response.status_code == status.HTTP_200_OK
...
async def test_post_log_non_blocking_accepted(
    client: httpx.AsyncClient, test_log_schema: LogCreateSchema, mocker
):
    """
    ...
    response = await client.post("/logs/non-blocking/", json=sample_record)
...
async def test_post_log_non_blocking_service_unavailable(
    ...
    test_log_schema: LogCreateSchema,
):
    """
    ...
    response = await client.post(
        "/logs/non-blocking/", json=test_log_schema.model_dump()
    )
